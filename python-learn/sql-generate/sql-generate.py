...
def sql_generate_hr(schema):

    list_res = []
    with open('sql-template-hr-src.txt', 'r', encoding='utf-8') as file_src:
        for line in file_src:   # 按行读取
            # print(line)
            list_res.append(line.strip())
    # print(list_res)
    with open('sql-template-hr-res.txt', 'w', encoding='utf-8') as file_res:
        for i in range(len(list_res)):
            sql = 'select * from' + ' ' + schema + '.' + list_res[i] + ';'
            file_res.write(sql + '\n')
    pass
